...
void main() => runApp(const FixMyRoadApp());
...
class _HomeState extends State<_Home> with TickerProviderStateMixin {
  ...
  Future<void> _pickDemoImage() async {
    final name = await _fakePicker(context, title: 'Upload Image', types: const ['.jpg', '.png', '.jpeg']);
    if (name == null) return;
    setState(() => _demoImageName = name);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Demo image selected: $name')));
    }
  }

  Future<void> _pickVideoAndAnalyze() async {
    final name = await _fakePicker(context, title: 'Upload Video', types: const ['.mp4', '.mov', '.mkv']);
    if (name == null) return;

    setState(() {
      _videoName = name;
      _latestReport = null; // clear old while "analyzing"
    });

    final report = await _simulateReport(videoName: name);
    if (!mounted) return;
    setState(() {
  ...
  Future<_VideoReport> _simulateReport({required String videoName}) async {
    ...
    final rng = Random();
    int frames = rng.nextInt(500) + 600; // 600–1099 frames
    int detections = rng.nextInt(12) + 7; // 7–18 potholes
    int high = max(1, (detections * (0.28 + rng.nextDouble() * 0.18)).round());
    int medium = max(1, (detections * (0.38 + rng.nextDouble() * 0.22)).round());
    int low = max(0, detections - high - medium);
    double confAvg = (0.74 + rng.nextDouble() * 0.22); // 0.74–0.96
                  ...
                  children: [
                    ...
                    Expanded(
                      child: _ActionButton(
                        ...
                        subtitle: _videoName == null ? 'Upload video' : 'Selected: $_videoName',
                        onPressed: _pickVideoAndAnalyze,
                        gradient: const LinearGradient(
  ...
  Widget _emptyReportPlaceholder(BuildContext context) {
    ...
    return _FrostedCard(
      child: ListTile(
        ...
        subtitle: const Text('Upload a video via Live Detection to generate an AI report.'),
      ),
    );
  }
  ...
  Widget _howToUseSection(BuildContext context, {Key? key}) {
    final steps = [
      ('Capture', 'Record a short, steady video of the road while driving slowly or walking safely.'),
      ('Upload', 'Tap Live Detection and select your video. We analyze it on-device/cloud.'),
      ('Review', 'See total potholes, severity split, confidence, and hotspot points.'),
      ('Report', 'Share or forward the summary to your local authorities/ward office.'),
      ('Improve', 'Add more videos or images to help prioritize maintenance.'),
    ];
    return Padding(
      key: key,
